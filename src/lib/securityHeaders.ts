...
export default function securityHeaders(app: Express) {
    app.use((req, res, next) => {
        ...
        // X-XSS-Protection: 0
        res.append("Content-Security-Policy:", "default-src 'self';base-uri 'self';font-src 'self' https: data:;form-action 'self';frame-ancestors 'self';img-src 'self' data:;object-src 'none';script-src 'self';script-src-attr 'none';style-src 'self' https: 'unsafe-inline';upgrade-insecure-requests")
